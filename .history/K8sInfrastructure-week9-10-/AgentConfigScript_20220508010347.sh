...
if [ -x "$(command -v systemctl)" ]; then
    ./config.sh --pool BuildPool --acceptteeeula --agent $HOSTNAME --url https://dev.azure.com/lihaybs3/ --work _work --projectname 'BOOTCAMP-APP K8S' --auth PAT --token tkna4y7bjlm2qeo6pooyvqwbqhjzj5ntm66cqzkvjwk6wcl4utsa --runasservice
    sudo ./svc.sh install
    sudo ./svc.sh start
else
    ./config.sh --pool BuildPool --acceptteeeula --agent $HOSTNAME --url https://dev.azure.com/lihaybs3/ --work _work --projectname 'BOOTCAMP-APP K8S' --auth PAT --token tkna4y7bjlm2qeo6pooyvqwbqhjzj5ntm66cqzkvjwk6wcl4utsa
    ./run.sh
fi
