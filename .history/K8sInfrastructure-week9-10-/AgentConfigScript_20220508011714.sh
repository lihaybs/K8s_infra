...
az aks update -n k8s-weight-tracker -g RG --attach-acr AcrForAppLihay
...
if [ -x "$(command -v systemctl)" ]; then
    ./config.sh --pool my-pool --acceptteeeula --agent $HOSTNAME --url https://dev.azure.com/lihaybs3/ --work _work --projectname 'BOOTCAMP-APP K8S' --auth PAT --token we3ibpqj7ip5i2hyzagc3q5zm3e6bfiou2xtk6i2rcxvh5al3osq --runasservice
    sudo ./svc.sh install
    sudo ./svc.sh start
else
    ./config.sh --pool my-pool --acceptteeeula --agent $HOSTNAME --url https://dev.azure.com/lihaybs3/ --work _work --projectname 'BOOTCAMP-APP K8S' --auth PAT --token we3ibpqj7ip5i2hyzagc3q5zm3e6bfiou2xtk6i2rcxvh5al3osq
    ./run.sh
fi
